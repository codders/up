...
const notifyUser: (arg0: string, arg1: Record<string, string>) => Promise<any> =
  function (target: string, message: Record<string, string>) {
    console.log(
    ...
    return loadSubscription(target)
      .then((token) => {
...
export const sendShowUpNotification: (
  arg0: up.UpRecordWithName,
) => Promise<any> = function (record: up.UpRecordWithName) {
  console.log(
    'Sending show up notification to user ' + record.inviteduid + ': ',
    record,
  );
...
export const sendUpMatchNotification: (
  arg0: string,
  arg1: up.UpRecord,
) => Promise<any> = function (senderName: string, record: up.UpRecord) {
  console.log(
    'Sending match notification to user ' + record.uid + ': ',
    record,
  );
